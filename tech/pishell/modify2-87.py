...
def process_file(file_path, content):
    ...
    if not encoding:
        print "无法检测文件编码: {}".format(file_path)
        return
    ...
    if html is None:
        print "无法读取文件: {}".format(file_path)
        return

    # 在<head>标签后的第一行插入内容
    if '<head>' in html.lower():
        updated_html = re.sub(r'<head>', u'<head>\n{}'.format(content), html, 1)
        if updated_html is None:
            print "正则替换失败: {}".format(file_path)
            return
        ...
        try:
            ...
            print "文件已更新: {}".format(file_path)
        except IOError:
            print "无法写入文件: {}".format(file_path)
    else:
        print "文件中没有<head>标签: {}".format(file_path)
